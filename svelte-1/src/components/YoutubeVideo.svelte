<script lang="ts">
  ...
  <template>
    <div class="flex youtube-thumbnail" on:click={showVideo} title={video.snippet.title}>
      <img 
      ...
      {:else}
      <button class='delete-fav' v-else on:click={deleteFavVideo}> Eliminar de favoritos</button>
      {/if}
    </div>
  </template>
